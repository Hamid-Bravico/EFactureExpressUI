        ...
        {showTooltip && (
          <div className="absolute z-50 w-80 p-4 mt-2 text-sm text-gray-700 bg-white border border-gray-200 rounded-lg shadow-lg -left-2 transform -translate-x-1/2">
            ...
            <div className="space-y-3">
              ...
              <div>
                <h4 className="font-medium text-gray-800 mb-2">{t('invoice.import.help.businessLogic')}</h4>
                <p className="text-xs text-gray-600 bg-yellow-50 p-2 rounded border border-yellow-200">
                  {t('invoice.import.help.businessLogicNote')}
                </p>
              </div>

              <div>
                <h4 className="font-medium text-gray-800 mb-2">{t('invoice.import.help.paymentMethods')}</h4>
                <ul className="space-y-1 text-gray-600">
                  {(t('invoice.import.help.paymentMethodValues', { returnObjects: true }) as string[]).map((method: string, index: number) => (
                    <li key={index} className="flex items-start">
