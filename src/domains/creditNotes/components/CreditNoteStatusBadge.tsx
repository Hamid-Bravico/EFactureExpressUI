...
const CreditNoteStatusBadge: React.FC<CreditNoteStatusBadgeProps> = ({ 
  status, 
  className = '', 
    ...
    switch (status) {
      case 0: // Draft
        return {
          ...
          text: t('creditNote.status.draft')
        };
      case 1: // Ready
        return {
          ...
          text: t('creditNote.status.ready')
        };
      case 2: // AwaitingClearance
        return {
          ...
          text: t('creditNote.status.awaitingClearance')
        };
      case 3: // Validated
        return {
          ...
          text: t('creditNote.status.validated')
        };
      case 4: // Rejected
        return {
          ...
          text: t('creditNote.status.rejected')
        };
      default:
        return {
          color: 'bg-gray-100 text-gray-800',
          icon: null,
          text: t('creditNote.status.unknown')
        };
    }
  };
    ...
    <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-sm font-medium ${config.color} ${className}`}>
      ...
      {status === 3 && dgiSubmissionId && (
        <button
          ...
          title={t('creditNote.actions.copyDgiId')}
        >
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
